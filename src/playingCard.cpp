...
std::string PlayingCard::toString() const {
  ...
  ss << "Rank: " << static_cast<int>(getRank())
     ...
     << " Seal: " << static_cast<int>(getSeal()) << " ID: " << getID();
  return ss.str();
}
...
PlayingCard::PlayingCard(int8_t rank, int8_t suit, int8_t enhancement,
                         int8_t edition, int8_t seal) {
...
void PlayingCard::setSuit(int8_t suit) { set(3, suit, SUIT_SHIFT, BIT_2MASK); }

void PlayingCard::setRank(int8_t rank) { set(13, rank, RANK_SHIFT, BIT_4MASK); }
...
void PlayingCard::setSeal(int8_t seal) { set(4, seal, SEAL_SHIFT, BIT_3MASK); }
